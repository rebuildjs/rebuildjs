...
export async function browser__build(config) {
  ...
  const path_a = await new fdir().glob("**/*.browser.{ts,js,tsx,jsx}").withFullPaths().crawl(app_path_(app_ctx)).withPromise();
...
export async function server__build(config) {
  ...
  const path_a = await new fdir().glob("**/*.server.{ts,js,tsx,jsx}").withFullPaths().crawl(app_path_(app_ctx)).withPromise();
  const entryPoints = esbuild__config?.entryPoints ?? [];
  for (const path of path_a) {
    entryPoints.push(path);
  }
  ...
  return {
    ...
    setup(build2) {
      build2.onEnd(async (result) => {
        if (result.metafile) {
          const { outdir } = build2.initialOptions;
        ...
        for (let output__relative_path in outputs) {
          if (/(\.js|\.mjs)(\.map)?$/.test(output__relative_path))
            continue;
          const asset_path = join(cwd_(app_ctx), output__relative_path);
          const link_path = join(browser_path_(app_ctx), relative(server__relative_path_(app_ctx), output__relative_path));
